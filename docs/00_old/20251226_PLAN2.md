...
### Phase 5: ドキュメント同期
- [x] `docs/requirement/requirement.md` の更新
- [x] README/INSTALL に影響があれば更新

### 追加: バックアップ（エクスポート/インポート）
- [x] サムネ込みでエクスポート（JSON）
- [x] インポート（他アカウントでも復元できるように、メモIDを再生成 + サムネをアップロード）
- [x] UI（`📦`）を追加

### 追加: 認証/同期（不具合修正）
- [x] アカウント切替時に旧ローカルデータが新アカウントへ同期されない（ダウンロードのみで開始）
