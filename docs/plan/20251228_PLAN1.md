...
**現在地**: Phase 2 - 字幕取得の不具合修正（拡張background fetch優先に切替 / 検証待ち）  
**次のアクション**: 拡張をビルド→Chromeで拡張を再読み込み→同じ動画で字幕が取れるか確認  
**ブロッカー**: なし（結果待ち）  
**変更ファイル**: `src/background/background.ts`, `server/index.mjs`, `package.json`, `docs/requirement/requirements.md`  
**追加成果物**: `docs/report/20251228_youtube-transcript-fetch-empty.md`, `docs/report/20251228_youtube-summary-extension-research.md`, `docs/report/20251228_youtube-transcript-blocked-alternatives.md`  
...
**重要な決定事項**: 字幕取得失敗時の詳細をAPIレスポンスに含める（debug用）  
**重要な決定事項**: 字幕取得のタイムアウトを延長し、youtubei側のみ1回リトライする  
**重要な決定事項**: timedtext取得のフォールバックとして`"captions":`分割パースとDOMParser解析を追加  
**重要な決定事項**: timedtext取得のreferrerをwatch URLに固定して空レスポンス対策  
**重要な決定事項**: timedtextの空レスポンス対策として、ページ内（MAIN world）fetchより拡張backgroundのfetchを優先する  
**重要な決定事項**: ログインが必要な動画でも取得できるよう、background fetchはYouTube Cookieを含めて実行する（`credentials: include`）  
**重要な決定事項**: watch HTMLの`baseUrl`が空を返すケースがあるため、空だった場合はInnerTube playerで`baseUrl`を取り直して再試行する  
**重要な決定事項**: InnerTube playerはCookie付きだと403になる場合があるため、まずCookieなし（`credentials: omit`）で呼び出して`baseUrl`を取り直す  
**重要な決定事項**: YouTubeのSPA遷移対策としてhistoryフック＋短時間スキャン再実行を追加  
