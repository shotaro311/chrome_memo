...
ブラウザ右上に表示される軽量なメモパネル（オーバーレイ）で、フォルダ分けされたメモと固定クイックメモを扱える Chrome 拡張機能を提供する。

### 1.2 コンセプト

- “今見ているページを離れず”にメモする（ページ上のオーバーレイ）
- ショートカットで素早くクイックメモに追記
...
- 閉じる：パネルの閉じるボタン
- 閉じた後の再表示：常に「クイックメモを前面表示」
- タブ内状態保持：
  - 同一タブ内のリロード/URL遷移でも保持する
  - タブが閉じられるまで保持（ブラウザ再起動・復元後はデフォルトに戻って良い）
  - 閉じる操作は「非表示」にするのみで、同一タブ再表示時はサイズ等の状態を復元する
...
- パネル表示時は常にクイックメモ本文末尾へフォーカス

### 4.1.5 構成

- 上部：固定クイックメモ領域（常に最上部に固定表示）
- 中部：フォルダ/メモ一覧、検索、メモ本文エリア
...
- 常に 1 件のみ
- パネル最上部に固定表示
- ショートカット追記先：常に固定クイックメモ
- クイックメモ → 新規メモ保存：
  - 保存先フォルダ選択あり
  - タイトル入力は任意（未入力時は日時ベースで自動タイトル）
  - コピー方式：保存後もクイックメモ本文は残す
